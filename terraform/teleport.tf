...
resource "teleport_role" "workload_id_demo_backend_2_bot_role" {
  version = "v7"
  metadata = {
    name = "workload-id-${var.backend_two_workload_name}-spiffe-bot"
  }

  spec = {
    allow = {
      spiffe = [{
          path = "/workload-id-demo/${var.backend_two_workload_name}"
          ip_sans = ["0.0.0.0/0"]
          dns_sans = ["*"]
      }]
    }
...
resource "teleport_bot" "workload_id_demo_backend_2_bot" {
  ...
  roles = [teleport_role.workload_id_demo_backend_2_bot_role.metadata.name]
}
